...
class UpdateStockSerialStatus extends Command {
     ...
     * @var string
     */
    protected $name = 'ebay:update-stock-serial-status';
     ...
     * @var string
     */
    protected $description = 'Update status to sold If Serial match from stock table with Custom Label in master ebay table.';
    ...
    public function handle() {
        ...
        $ItemSku = EbayOrderItems::with(['order', 'stock_serial'])
                ->has('stock_serial', ">", 0)
                ->where("owner", "")
                ->where("item_sku", "!=", "")
                ->get();
            ...
            foreach ($ItemSku as $Item) {
                if ($Item->stock_serial->count() > 0) {
                    ...
                    if (!in_array($Item->order->status, [\App\EbayOrders::STATUS_CANCELLED, \App\EbayOrders::STATUS_REFUNDED])) {
                        $stockSave = $Item->stock_serial;

                        if (in_array($Item->stock_serial->status, [Stock::STATUS_RETAIL_STOCK, Stock::STATUS_IN_STOCK, Stock::STATUS_READY_FOR_SALE, Stock::STATUS_LISTED_ON_AUCTION])) {
                        ...
                        if (in_array($Item->stock_serial->status, [Stock::STATUS_RETAIL_STOCK, Stock::STATUS_IN_STOCK, Stock::STATUS_READY_FOR_SALE, Stock::STATUS_LISTED_ON_AUCTION])) {
                            StockLog::create([
                                ...
                                'content' => 'Changed "Status" from ' . $Item->stock_serial->status . ' to ' . Stock::STATUS_SOLD,
                            ]);
                        }
                    }
                }
