...
class CreateChecks extends Command {
	 ...
	 * @var string
	 */
	protected $name = 'phone-check:create-checks';
			    ...
			    if(!isset($res->status)){
                    ...
                    foreach ($res as $report) {
                       ...
                       if(isset($report->master_id)){
                           ...
                           if($report->LPN && !empty($report->LPN)){
                               $stock = Stock::where('third_party_ref', $report->LPN)->first();
                           }elseif (strlen($report->IMEI) != 15 && !is_numeric($report->IMEI)) {
