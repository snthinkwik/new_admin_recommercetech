...
class EmailSend implements ShouldQueue
{
    ...
    public function __construct($sale,$type,$customer,$invoicePath,$auction=null)
    {
        ...
        $this->fullName= $this->newCustomer['first_name'].''. $this->newCustomer['last_name'];
    ...
    public function handle()
    {
        ...
        if($this->type===self::EMAIL_CREATED){

            $this->saleCreatedEmail();
        }elseif ($this->type===self::EMAIL_CANCELLED){
...
//    }
    ...
    protected function saleCancelledEmail()
    {
        ...
        if(!empty($this->newCustomer)) {
            alert("Can't send email to customer for sale \"{$this->sale->id}\" - customer not found.");
            return;
        }

        if (!$this->newCustomer['email']) {
            alert("Can't send email to customer for sale \"{$this->sale->id}\" - email address empty.");
            return;
        }

        Mail::send(
            $this->typeToView[$this->type],
            ['customer'=>$this->newCustomer,'sale'=>$this->sale],
            function(Message $mail)  {
                $mail->subject("Order with " . config('app.company_name') . " has been cancelled - #$this->sale->invoice_number")
                    ->to($this->newCustomer['email'], $this->fullName)
                    ->bcc(config('mail.sale_cc.address'), config('mail.sale_cc.name'))
                    ->from(config('mail.sales_address'), config('mail.from.name'))
        ...
        SaleLog::create([
            ...
            'content' => "Email sent, subject: 'Order with ".config('app.company_name')." has been cancelled - #$this->sale->invoice_number'"
        ]);
    }
...
//        $invoicePath = $this->invoicePath;
        ...
        $invoiceNumber=$this->sale->invoice_number.'-'.$this->sale->invoice_doc_number;
        Mail::send(
            $this->typeToView[$this->type],
            ['customer'=>$this->newCustomer,'sale'=>$this->sale],
            function(Message $mail) use($invoiceNumber) {
        ...
        $user = User::where('invoice_api_id', $this->newCustomer['external_id'])->first();
        $phone = !is_null($this->newCustomer['phone'])  ? : $user->phone;

        $name = $this->newCustomer['first_name'];
        $message = "Hi $name, thank you for your order with Recomm. We have now emailed your invoice to you. Order total:". $this->sale->amount_formatted. "Regards, Chris";
        if($phone) {
            $txtlocal = app('App\Contracts\Txtlocal');
