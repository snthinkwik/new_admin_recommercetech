class eBayFee
{
    constructor()
    {
        ...
        this.bindMethods();
        this.bindEvents();
        $("#manually-assign-button").on('click',function () {
            var owner=$("#owner").val();
            var company_name = [];
            ...
            else if (!confirm("Are you sure you want to update companies?")) {
                return;
            }

            this.loadXhr = $.ajax({
                url: CURRENT_URL +'/manual-fee-assignment',
                type: 'post',
                data:{ids:company_name, owner:owner},
    ...
    cacheDom()
    {
        this.$queryForm = $('#ebay-fee-search-form');
        this.$queryInput = $('#ebay-free-order_number');
        this.$querySort = $('#ebay-fee-sort');
        this.$itemsWrapper = $('#ebay-fee-items-wrapper');
        this.$paginationWrapper = $('#ebay-fee-pagination-wrapper');
