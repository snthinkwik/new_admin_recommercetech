...
?>
	<div>
		@foreach($items as $item)
			{{ $item->quantity }}x @foreach($requestedFields as $index => $field) {{ $index != 1 ? "-":null }} {{ in_array($field, $fieldsFormatted) ? money_format(config('app.money_format'), $item->{$field} ) : $item->{$field} }} @endforeach<br/>
		@endforeach
	</div>
