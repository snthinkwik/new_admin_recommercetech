...
<div class="container">
    <div class="row mb-4">
        ...
        <div class="col-lg-6 text-right d-inline-block text-bold">
            <div class="row">
                <div class="col-lg-6">
                    <p class="mb-0">Net to Invoice: <span class="text-success mr-2">{{money_format(config('app.money_format'), $SalesPrice-$Fees-$PayPalPrice-$totalDeliveryCharge-$totalEbayDeliveryCharge)}}</span><i class="fa fa-info-circle" data-toggle="tooltip" data-placement="bottom" html=true title="Net to Invoice = Total to Invoice - Total Fees - Total PayPal Fees - Total Delivery Charges (DPD) - Total Ebay Delivery Charges (Royal Mail 1st & Hermes)"></i> </p>
                    <p class="mb-0">Total to Invoice: <span class="text-success mr-2">{{money_format(config('app.money_format'), $SalesPrice)}}</span><i class="fa fa-info-circle" data-toggle="tooltip" data-placement="bottom" html=true title="Sum of Individual Item Price from Order Item table Where Order status is Dispatched AND Order Item has Fees AND No Invoice Number AND Owner is Recomm"></i></p>
                    <p class="mb-0">Total Fees: <span class="text-success mr-2">{{money_format(config('app.money_format'), $Fees)}}</span><i class="fa fa-info-circle" data-toggle="tooltip" data-placement="bottom" html=true title="Sum of Fees from eBay Fee table Where No Invoice Number AND Owner is Recomm AND Matched is Yes"></i></p>
                </div>
                <div class="col-lg-6">
                    <p class="mb-0">Total PayPal Fees: <span class="text-success mr-2">{{money_format(config('app.money_format'), $PayPalPrice)}}</span><i class="fa fa-info-circle" data-toggle="tooltip" data-placement="bottom" html=true title="Sum of PayPal Fees from Main Order table Where Owner is Recomm AND Status is in 'Dispatched','Refunded','Cancelled'"></i></p>
                    <p class="mb-0">Total Delivery Charges: <span class="text-success mr-2">{{money_format(config('app.money_format'), $totalDeliveryCharge + $totalEbayDeliveryCharge)}}</span><i class="fa fa-info-circle" data-toggle="tooltip" data-placement="bottom" html=true title="Sum of Cost from DPD Import table Where Owner is Recomm + Sum of Cost from eBay Delivery Charge table Where Owner is Recomm"></i></p>
                    ...
                    <p class="mb-0">Total Amount Refunded: <span class="text-success mr-2">{{money_format(config('app.money_format'), $totalRefundAmount)}}</span><i class="fa fa-info-circle" data-toggle="tooltip" data-placement="bottom" html=true title="Sum of refunded amount from ebay refund table"></i></p>
                </div>
            </div>            
        </div>
    </div>
